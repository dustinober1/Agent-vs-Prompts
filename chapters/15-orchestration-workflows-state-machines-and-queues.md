...
The policy brief workflow uses a sequential state machine with checkpoints at each boundary. This ensures that transient failures (like search API timeouts) don't lose progress, and that the agent can resume from the last known-good artifact.

![Workflow State Machine A](../artifacts/workflow_state_machine_A.md)

### Instructional Design (Case Study B)

The training module workflow requires a more complex state machine to handle parallel generation of scenarios and assessments, as well as multiple human approval gates.

![Workflow State Machine B](../artifacts/workflow_state_machine_B.md)
