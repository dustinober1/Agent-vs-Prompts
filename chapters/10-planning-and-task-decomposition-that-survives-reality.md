...
### Research+Write (Case Study A): The Linear-Adaptive Plan

For the policy brief, we use a plan with explicit checkpoints after research, extraction, and drafting. This ensures that the agent doesn't build a brief on top of archived or irrelevant documents.

![Planning Template A](../artifacts/planning_template_A.md)

### Instructional Design (Case Study B): The Hierarchical Plan

The training module requires a hierarchical plan because each phase (Objectives, Content, QA) has its own complex sub-tasks and gates.

![Planning Template B](../artifacts/planning_template_B.md)
