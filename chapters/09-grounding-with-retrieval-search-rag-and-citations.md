...
The policy brief agent needs retrieval that's both thorough and auditable. We use a hybrid retrieval pipeline and a strict citation policy to ensure every claim is grounded.

![Retrieval Map A](../artifacts/retrieval_map_A.md)

### Instructional Design (Case Study B)

The training module agent needs retrieval that ensures accuracy and avoids licensing issues. We enforce a freshness policy and map every learning objective to its policy source.

![Retrieval Map B](../artifacts/retrieval_map_B.md)
