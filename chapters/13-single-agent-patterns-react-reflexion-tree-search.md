...
The brief writer uses **ReAct** to interleave its reasoning with specific tool calls, ensuring every decision is grounded in real logs and policies.

![ReAct Trace A](../artifacts/react_trace_A.md)

When the initial draft fails our strict quality rubrics, the agent uses a **Reflexion** protocol to diagnose the gap and remediate it autonomously before delivery.

![Reflexion Protocol A](../artifacts/reflexion_protocol_A.md)
